...
void GameOfLife::drawPattern(PatternType type) {
    ...
    switch (type) {
        ...
        case PatternType::SpaceShip: {
            ...
            cells[indexTop + 3] = 1;
            cells[indexMiddle] = 1;
            cells[indexMiddle + 3] = 1;
            cells[indexBottom + 3] = 1;
            cells[indexBottom2 + 3] = 1;
            cells[indexBottom3] = 1;
            cells[indexBottom3 + 2] = 1;
            break;
        }
        ...
        case PatternType::Pentadecathlon: {
            ...
            cells[indexMiddle + 1] = 1;
            cells[indexMiddle + 3] = 1;
            cells[indexMiddle + 4] = 1;
            cells[indexMiddle + 5] = 1;
            cells[indexMiddle + 6] = 1;
            cells[indexMiddle] = 1;
            cells[indexMiddle - 1] = 1;
            cells[indexMiddle - 3] = 1;
            cells[indexMiddle - 4] = 1;
        ...
        for (int i = 1; i < latticeHeight - 1; i++) {
            for (int j = 1; j < latticeWidth - 1; j++) {
                ...
                if (j == 0 && i != 0 && i != latticeHeight - 1) {
                    neighbors = cells[index_above + latticeWidth - 1] + cells[index_above] + cells[index_above + 1] + cells[index + latticeWidth - 1] + cells[index + 1] + cells[index_below + latticeWidth - 1] + cells[index_below] + cells[index_below + 1];
                }
                // if the cells are on the extreme right of the lattice consider the cells on the extreme left as their neighbors:ignoring the 4 corner cells
                    ...
                    neighbors = cells[index_above - 1] + cells[index_above] + cells[index_above - latticeWidth + 1] + cells[index - 1] + cells[index - latticeWidth + 1] + cells[index_below - 1] + cells[index_below] + cells[index_below - latticeWidth + 1];
                }
                ...
                //for any other cell on the lattice
                    ...
                    neighbors = cells[index_above - 1] + cells[index_above] + cells[index_above + 1] + cells[index - 1] + cells[index + 1] + cells[index_below - 1] + cells[index_below] + cells[index_below + 1];
                }

                // declare the cell alive or dead based on the rules below - check Wikipedia for these rules
                if ((cells[index] == 1) && (neighbors < 2))
                    temp_cells[index] = 0;
                else if ((cells[index] == 1) && (neighbors > 3))
                    temp_cells[index] = 0;
                else if ((cells[index] == 0) && (neighbors == 3))
                    temp_cells[index] = 1;
                else
